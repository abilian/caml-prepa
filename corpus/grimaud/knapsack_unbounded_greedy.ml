(* From "Informatique MPI", by Aslı Grimaud and Gilles Grimaud.
   ...
   solutions/Grimaud_UnboundedKnapsack_greedy.ml
...
let unboundedknapsack_greedy (ks : knapsack) : solution =
  (* Array to store the decision *)
  let lambda = Array.make ks.n 0.0 in
  (* Fill the knapsack greedily *)
  let rec fill i weight_left =
    if i < ks.n then
      let weight = ks.weights.(i) in
      let count = weight_left / weight in
      lambda.(i) <- float_of_int count;
      fill (i + 1) (weight_left - count * weight)
  in
  fill 0 ks.w;
  lambda
...
let () =
  ...
  let sol = unboundedknapsack_greedy sorted_ks in
